...
@customElement('poe-fragment-stash-list')
export class PoeFragmentStashListElement extends LitElement {
  ...
  private async loadPrices(): Promise<void> {
    try {
      const rows = await this.stashLoader.fragmentPrices(this.league as any);
      ...
      this.prices = new Map();
      const msg = typeof err === 'string' ? err : err instanceof Error ? err.message : 'Failed to fetch fragment prices';
  ...
  private renderHeader(cols: string[]): TemplateResult {
    const keys: Record<string, PoeFragmentStashListElement['sortBy']> = {
  ...
  private onSort(col: PoeFragmentStashListElement['sortBy']) {
    if (this.sortBy === col) {
...
declare global {
  interface HTMLElementTagNameMap {
    'poe-fragment-stash-list': PoeFragmentStashListElement;
  }
}
